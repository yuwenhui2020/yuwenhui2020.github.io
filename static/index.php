...
function is_func($func){
	return function_exists($func) ? 有 : 无;
}
...
function isfun($funName = '')
{
	...
	return (false !== function_exists($funName)) ? '<font color="green">是</font>' : '<font color="red">否</font>';
}
...
<!--函数检测-->
<table width="100%" cellpadding="3" cellspacing="0" style="text-align: center">
	<tr><th colspan="3" type="submit" class="th_4" >函数检测</th></tr>
  <tr>
    <td>
      请输入您要检测的函数：
...
<table width="100%" cellpadding="4" cellspacing="0" style="text-align: center">
  <tr><th colspan="4" class="th_5">邮件发送检测</th></tr>
  <tr>
  <td width="15%">管理员邮箱</td>
	  <td width="20%"><?php echo $_SERVER['SERVER_ADMIN'];?></td>
...
<td>字符转换函数支持</td>
		<td>iconv <?php echo is_func('iconv')?> 或 mbstring <?php echo is_func('mb_convert_encoding')?></td>
  </tr>
  <tr>
    <td>PHP运行方式：</td>
  ...
  <tr>
    <td>Cookie 支持：</td>
    <td><?php echo isset($_COOKIE)?'<font color="green">是</font>' : '<font color="red">否</font>';?></td>
   ...
   <tr>
    <td>SMTP支持：</td>
    <td><?php echo get_cfg_var("SMTP")?'<font color="green">是</font>' : '<font color="red">否</font>';?></td>
    <td>SMTP地址：</td>
    <td><?php echo get_cfg_var("SMTP")?get_cfg_var("SMTP"):'<font color="red">否</font>';?></td>
  </tr> 
	<tr>
		<td>默认支持函数（enable_functions）：</td>
		<td><a href='<?php echo $phpSelf;?>?act=Function' target='_blank' class='static'>请点这里查看详细！</a></td>		
...
<table width="100%" cellpadding="3" cellspacing="0" style="text-align: center">
  ...
  <tr>
    ...
    <td width="18%"><?php $zend_version = zend_version();if(empty($zend_version)){echo '<font color=red>否</font>';}else{echo $zend_version;}?></td>
    <td width="32%">
<?php
...
?>
    ...
    <td width="18%"><?php if($PHP_VERSION > 2){echo (get_cfg_var("zend_loader.enable"))?'<font color=green>是</font>':'<font color=red>否</font>';} else{if(function_exists('zend_optimizer_version')){	echo zend_optimizer_version();}else{	echo (get_cfg_var("zend_optimizer.optimization_level")||get_cfg_var("zend_extension_manager.optimizer_ts")||get_cfg_var("zend.ze1_compatibility_mode")||get_cfg_var("zend_extension_ts"))?'<font color=green>是</font>':'<font color=red>否</font>';}}?></td>
  </tr>
  <tr>
    <td>eAccelerator</td>
    <td><?php if((phpversion('eAccelerator'))!=''){echo phpversion('eAccelerator');}else{ echo "<font color=red>否</font>";} ?></td>
    <td>ioncube</td>
    <td><?php if(extension_loaded('ionCube Loader')){   $ys = ioncube_loader_iversion();   $gm = ".".(int)substr($ys,3,2);   echo ionCube_Loader_version().$gm;}else{echo "<font color=red>否</font>";}?></td>
  </tr>
  <tr>
    <td>XCache</td>
    <td><?php if((phpversion('XCache'))!=''){echo phpversion('XCache');}else{ echo "<font color=red>否</font>";} ?></td>
    <td>APC</td>
    <td><?php if((phpversion('APC'))!=''){echo phpversion('APC');}else{ echo "<font color=red>否</font>";} ?></td>
  </tr>
</table>
...
<table width="100%" cellpadding="3" cellspacing="0" style="text-align: center">
  ...
  <tr>
    <td>SQLite 数据库：</td>
    <td><?php if(extension_loaded('sqlite3')) {$sqliteVer = SQLite3::version();echo "SQLite3　Ver ";echo $sqliteVer[versionString];}else {echo isfun("sqlite_close");if(isfun("sqlite_close") == '<font color="green">是</font>') {echo "&nbsp; 版本： ".@sqlite_libversion();}}?></td>
    <td>Hyperwave 数据库：</td>
